...
class RegionPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        region_affine = F.softmax(region / self.temperature, dim=2)
        region_heatmap = torch.sigmoid(region / self.temperature)

        region_affine = region_affine.view(*final_shape)
        region_heatmap = region_heatmap.view(*final_shape)

        region_params = self.region2affine(region_affine)
        region_params['heatmap'] = region_heatmap
